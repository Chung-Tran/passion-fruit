            ...
            <div className="container my-10">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-6 min-h-[600px] gap-8">
                        ...
                        <div className="lg:col-span-3 md:p-8 flex flex-col">
                            <span className="inline-block bg-green-100 text-green-700 text-sm rounded-full mb-4 w-fit px-6 py-1">
                                {languageText.productCode}: {product.code}
                            </span>
                            <h1 className="text-3xl font-bold text-gray-900 mb-6">{product.name}</h1>

                            {/* Product Details */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {[{ icon: Package, label: languageText.unit, value: product.unit },
                                { icon: Apple, label: languageText.type, value: product.type },
                                { icon: Thermometer, label: languageText.storageTemp, value: product.storageTemp },
                                { icon: Clock, label: languageText.expiry, value: product.expiry },
                                { icon: Package, label: languageText.origin, value: product.origin },
                                { icon: Palette, label: languageText.color, value: product.color },]
                                    .map((item, idx) => (
                            ...
                            <button className="mt-8 w-full bg-green-600 text-white py-4 rounded-xl hover:bg-green-700 transition-all flex items-center justify-center space-x-2 shadow-md">
                                <a href={`tel:0347238240`} className="font-medium">{languageText.contactOrder}</a>
                    ...
                    <div className="border-t mt-6">
                        <div className=" py-6">
                            <div className="flex space-x-8 border-b">
                                {[
                                    { key: 'description', label: languageText.description },
                                    { key: 'process', label: languageText.process }]
                                    .map(tab => (
                                        <button
                                            key={tab.key}
                ...
                <section className="mx-auto py-8">
                    <div className="text-center mb-12">
                        <h2 className="textTitle mx-auto">{languageText.relatedProducts}</h2>
                    </div>
